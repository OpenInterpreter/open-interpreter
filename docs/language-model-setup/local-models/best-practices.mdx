...
interpreter.llm.model = "openai/x" # Tells OI to send messages in OpenAI's format. Only used with LM Studio.
interpreter.llm.api_key = "fake_key" # LiteLLM, which we use to talk to LM Studio, requires this to use LiteLLM
