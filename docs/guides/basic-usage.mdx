...
>
  Try TestDriver without installing anything on your computer
</Card>
...
>
  An example implementation of TestDriver's streaming capabilities
</Card>
...
In your terminal, TestDriver behaves like ChatGPT and will not remember previous conversations. Simply run `interpreter` to start a new chat:

```shell
interpreter
```

In Python, TestDriver remembers conversation history. If you want to start fresh, you can reset it:
...
In your terminal, TestDriver will save previous conversations to `<your application directory>/TestDriver/conversations/`.
...
<Info>**Note:** The Python library will also inherit settings from this config file, but you can only change it by running `interpreter --config` or navigating to `<your application directory>/TestDriver/config.yaml` and editing it manually.</Info>
...
In Python, you can inspect and configure TestDriver's system message to extend its functionality, modify permissions, or give it more context.
...
TestDriver uses [LiteLLM](https://docs.litellm.ai/docs/providers/) to connect to language models.
