...
Specifies which language model to use. Check out the [models](/language-models/) section for a list of available models. TestDriver uses [LiteLLM](https://github.com/BerriAI/litellm) under the hood to support over 100+ models.
...
Inform TestDriver that the language model you're using supports function calling.
...
Inform TestDriver that the language model you're using does not support function calling.
...
Inform TestDriver that the language model you're using supports vision. Defaults to `False`.
...
Get the current installed version number of TestDriver.
...
Runs TestDriver in a loop, requiring it to admit to completing or failing every task.
...
```bash Terminal
interpreter --system_message "You are TestDriver..."
```
...
```yaml Profile
system_message: "You are TestDriver..."
```
...
Running the `computer` in offline mode will disable some online features, like the hosted [Computer API](https://api.testdriver.ai/). Inherits from `interpreter.offline`.
