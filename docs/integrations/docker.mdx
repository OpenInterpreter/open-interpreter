...
Docker support is currently experimental. Running TestDriver inside of a Docker container may not function as you expect. Let us know on [Discord](https://discord.com/invite/6p3fD6rBVm) if you encounter errors or have suggestions to improve Docker support.

We are working on an official integration for Docker in the coming weeks. For now, you can use TestDriver in a sandboxed Docker container environment using the following steps:
...
3. Run the following commands in the same directory to start TestDriver.
