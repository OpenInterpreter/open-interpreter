...
To build this integration, you just need to replace TestDriver's `python` (which runs locally) with a `python` that runs on E2B.
...
Then, define a custom language for TestDriver. The class name doesn't matter, but we'll call it `PythonE2B`:
...
class PythonE2B:
    """
    This class contains all requirements for being a custom language in TestDriver:
...
# Give TestDriver its languages. This will only let it run PythonE2B:
