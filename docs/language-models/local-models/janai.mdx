...
To run TestDriver with Jan.ai, follow these steps:
...
5. Now we fire up TestDriver with this custom model. Either run `interpreter --local` in the terminal to set it up interactively, or run this command, but replace `<model_id>` with the id of the model you downloaded:
